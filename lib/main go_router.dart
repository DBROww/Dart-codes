...
import 'package:hello_world/main%20TextEditController.dart';
import 'pages/produto_page.dart';
import 'pages/carrinho_page.dart';

// MODELO DO PRODUTO
class Suculenta { // Classe que representa uma suculenta da loja. Funciona como um "molde" para criar produtos.
  final int id;
  final String nome;
  ...
  const Suculenta({ // Construtor da classe. O "required" obriga que todos os campos sejam preenchidos ao criar uma suculenta.
    required this.id,
    required this.nome,
...
class CarrinhoController extends ChangeNotifier {
  ...
  void adicionar(Suculenta produto) {
    for (var item in itens) { // Percorre todos os itens do carrinho.
      if (item.produto.id == produto.id) { // Verifica se o produto já existe no carrinho.
  ...
  // REMOVER PRODUTO
  void remover(Suculenta produto) { // Remove uma unidade do produto.
    for (int i = 0; i < itens.length; i++) { // Percorre todos os itens da lista.
      if (itens[i].produto.id == produto.id) { // Verifica se encontrou o produto.
        if (itens[i].quantidade > 1) { // Se houver mais de uma unidade.
          itens[i].quantidade--; // Diminui apenas uma unidade.
        } else { // Se houver apenas uma unidade, remove completamente o item.
          itens.removeAt(i);
        }
  ...
  void removerItemCompleto(Suculenta produto) { // Remove todas as unidades de um produto de uma única vez.
    itens.removeWhere( // removeWhere percorre a lista e remove todos os elementos que atendam à condição.
      (item) => item.produto.id == produto.id, // Verifica se o ID do item é igual ao ID do produto recebido como parâmetro.
    );
  ...
  double get total { // Getter utilizado para calcular o valor total de todos os produtos do carrinho.
    double soma = 0;
    for (var item in itens) { // Percorre todos os itens do carrinho.
      soma += item.produto.preco * item.quantidade; // Multiplica o preço do produto pela quantidade e adiciona ao total acumulado.
    }
    return soma;
  }

  // QUANTIDADE TOTAL DE ITENS
  int get totalItens { // Getter que calcula quantos itens existem no carrinho.
    int soma = 0;
  ...
  // LIMPAR CARRINHO
  void limpar() { // Remove todos os produtos do carrinho.
...
final carrinho = CarrinhoController(); // Cria um único carrinho compartilhado por todo o aplicativo.
...
final GoRouter router = GoRouter(
  ...
  routes: [
    ...
    GoRoute(
      ...
      builder: (context, state) { // Obtém o parâmetro enviado pela URL.
        final id = int.parse(state.pathParameters['id']!); // Procura dentro da lista de produtos aquele cujo ID seja igual ao recebido.
        final produto = produtos.firstWhere((p) => p.id == id); // Abre a página de detalhes do produto.
  ...
  Widget build(BuildContext context) {
    return Scaffold( // cria a estrutura básica da tela
      appBar: AppBar(
        ...
        actions: [ // Área de ações da AppBar.
          ...
          ListenableBuilder(
            ...
            builder: (context, _) {
              return Stack( // Stack permite empilhar widgets.
                children: [
                  ...
                  // BADGE DE QUANTIDADE
                  if (carrinho.totalItens > 0) // Só aparece se existir pelo menos um item no carrinho.
                    Positioned(
                      right: 6,
                      top: 6,
                      child: Container(
                        ...
                        child: Text('${carrinho.totalItens}', // Quantidade total de itens.
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) { // Responsável por construir cada item.
          final produto = produtos[index]; // Obtém o produto atual.
          return Card( // Card utilizado para destacar visualmente cada produto da lista.
            margin: const EdgeInsets.only(
              bottom: 12,
            ),
            child: ListTile( // ListTile organiza automaticamente
              ...
              subtitle: Column(
                ...
                children: [
                  ...
                  Text('R\$ ${produto.preco.toStringAsFixed(2)}'), // Preço do produto.
                ],
              ),

              isThreeLine: true, // Informa que o subtítulo possui mais de uma linha.
